...
class ClanCommands(commands.Cog):
    ...
    async def _process_clan_rewards(self):
        ...
        for clan_id, clan_data in data_manager.clan_database.items():
            ...
            money_reward = len(clan_data["members"]) * 1000
            xp_reward = len(clan_data["members"]) * 50
    ...
    async def ranking_clans(self, interaction: discord.Interaction):
        ...
        if not sorted_clans:
            ...
            for i, clan_data in enumerate(sorted_clans[:10]):
                leader_id = clan_data.get("leader")
                leader_name = "Líder Desconhecido"
                if leader_id:
                    leader_name = await self._get_user_display_name(int(leader_id))

                embed.add_field(
                    name=f"#{i + 1} - {clan_data.get('name', 'Clã Desconhecido')}",  # Usa .get() para 'name'
                    value=(
                        f"Líder: {leader_name}\n"
                        f"Membros: {len(clan_data.get('members', []))}\n"  # Usa .get() para 'members'
                        f"XP: {clan_data.get('xp', 0):,}\n"  # Usa .get() para 'xp'
                        f"Dinheiro: {display_money(clan_data.get('money', 0))}"  # Usa .get() para 'money'
                    ),
                    inline=False,
                )
        await interaction.followup.send(embed=embed)
